...
SUDOERS=$( DPRINT cat /etc/sudoers /etc/sudoers.d/*  | grep -vE '#|Defaults|^\s*$' | grep -vE '(Cmnd_Alias|\\)' | awk '{gsub("%", ""); print}' )
...
if checkService "$SERVICES"  'cockpit' | grep -qi "is on this machine"; then
	...
	printf "${ORAG}[!] WE PROBABLY SHOULD KILL COCKPIT${NC}"
fi
